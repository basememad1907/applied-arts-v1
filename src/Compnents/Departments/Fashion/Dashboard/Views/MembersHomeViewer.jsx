...
export default function MembersHomeViewer() {
    ...
    useEffect(() => {
        if (localStorage.getItem("ourFashionMembersAtHome") === null) {
            localStorage.setItem("ourFashionMembersAtHome", JSON.stringify([]));
            setMembersContainer([]);
        } else {
            setMembersContainer(JSON.parse(localStorage.getItem("ourFashionMembersAtHome")));
        }
    }, []);
