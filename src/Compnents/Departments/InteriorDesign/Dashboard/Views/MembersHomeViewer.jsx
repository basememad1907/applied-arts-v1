...
export default function MembersHomeViewer() {
    ...
    useEffect(() => {
        if (localStorage.getItem("ourIDMembersAtHome") === null) {
            localStorage.setItem("ourIDMembersAtHome", JSON.stringify([]));
            setMembersContainer([]);
        } else {
            setMembersContainer(JSON.parse(localStorage.getItem("ourIDMembersAtHome")));
        }
    }, []);
