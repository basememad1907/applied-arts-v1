...
export default function MembersHomeViewer() {
    ...
    useEffect(() => {
        if (localStorage.getItem("ourPDMembersAtHome") === null) {
            localStorage.setItem("ourPDMembersAtHome", JSON.stringify([]));
            setMembersContainer([]);
        } else {
            setMembersContainer(JSON.parse(localStorage.getItem("ourPDMembersAtHome")));
        }
    }, []);
